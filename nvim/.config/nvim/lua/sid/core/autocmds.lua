...
vim.api.nvim_create_user_command('ViewFolds', function(opts)
	local arg = opts.args:lower()
	if arg == 'hide' then
		vim.wo.foldcolumn = '0'
		print('Fold column disabled')
	elseif arg == 'show' then
		vim.wo.foldcolumn = '1'
		print('Fold column enabled')
	else
...
end, { nargs = '?', desc = 'Toggle fold column visibility (show/hide)' })
